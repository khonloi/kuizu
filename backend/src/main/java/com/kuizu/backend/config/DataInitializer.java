    ...
    public CommandLineRunner initData() {
        return args -> {
            ...
            flashcardRepository.saveAll(List.of(
                    Flashcard.builder().flashcardSet(javaBasics).term("JVM").definition("Java Virtual Machine")
                            .orderIndex(0).isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(javaBasics).term("JDK").definition("Java Development Kit")
                            .orderIndex(1).isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(javaBasics).term("JRE").definition("Java Runtime Environment")
                            .orderIndex(2).isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(javaBasics).term("Polymorphism").definition("One name, many forms")
                            .orderIndex(3).isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(javaBasics).term("Inheritance")
                            .definition("Reusing code from parent classes").orderIndex(4).isDeleted(false).build()));

            flashcardRepository.saveAll(List.of(
                    Flashcard.builder().flashcardSet(calculusSum).term("d/dx (x^n)").definition("n * x^(n-1)")
                            .orderIndex(0).isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(calculusSum).term("d/dx (sin x)").definition("cos x").orderIndex(1)
                            .isDeleted(false).build(),
                    Flashcard.builder().flashcardSet(calculusSum).term("d/dx (e^x)").definition("e^x").orderIndex(2)
                            .isDeleted(false).build()));
...
//                    .build());
            ...
            folderSetRepository.save(FolderSet.builder()
                    .id(new FolderSet.FolderSetId(teacherFolder.getFolderId(), javaBasics.getSetId()))
            ...
            folderSetRepository.save(FolderSet.builder()
                    .id(new FolderSet.FolderSetId(teacherFolder.getFolderId(), calculusSum.getSetId()))
                    .folder(teacherFolder)
